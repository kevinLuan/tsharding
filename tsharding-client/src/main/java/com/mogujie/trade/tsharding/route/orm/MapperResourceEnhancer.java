...
public class MapperResourceEnhancer extends MapperEnhancer {
  ...
  private SQLEnhancerHandler getShardingTableHander(MappedStatement ms) {
    Class<?> clazzClass = getMapperClass(ms.getId());
    DataSourceRouting sharding = clazzClass.getAnnotation(DataSourceRouting.class);
    Class<?> enhancerHanderClass = sharding.sqlEnhancerHandler();
    try {
      SQLEnhancerHandler hander = (SQLEnhancerHandler) enhancerHanderClass.getConstructor(Class.class)
  ...
  private void processDynamicSqlSource(SQLEnhancerHandler hander, DynamicSqlSource sqlSource,
  ...
  private void processSqlNode(SQLEnhancerHandler hander, SqlNode sqlNode, Long shardingPara)
      throws Exception {
    try {
   ...
   * @throws Exception
   */
  private void processTextSqlNode(SQLEnhancerHandler hander, StaticTextSqlNode textSqlNode,
      Long shardingPara) throws Exception {
  ...
  public SqlSource enhancedShardingSQL(MappedStatement ms, Configuration configuration,
      Long shardingPara) {
    SQLEnhancerHandler hander = getShardingTableHander(ms);
